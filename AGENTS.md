...
config/fresh-start.json  optional automatic fresh starts for this home's second mates and a deduplicated /stow-then-/clear suggestion for the primary; absent = off; LOCAL, gitignored, and not inherited; see docs/configuration.md "Automatic fresh starts"
...
state/               runtime records and signals; gitignored
  ...
  fresh-start.check.sh fresh-start/  generated automatic fresh-start poll shim (armed at session start only while config/fresh-start.json exists) and its offer, attempt, and cooldown records; bin/fm-fresh-start.sh owns both
...
3. For `check:`, act on the named poll result, including merges, contribution signals, Relay events, process-to-event source results, and captain inbox notes; a handled inbox note is also acknowledged with `bin/fm-inbox.sh drain --ack <id>`, or it stays counted as still waiting for firstmate.
   ...
   A `fresh start due` check names the exact command to run in the background, with no captain message; a `fresh-start suggestion` check is relayed to the captain as that one short suggestion (`bin/fm-fresh-start.sh` owns both).
