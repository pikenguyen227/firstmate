...
| `fm-secondmate-restart.sh` | Persist open conversational work, then restart eligible second mates or report the fallback outcome; `--fresh-start` is the automatic fresh start's mode |
| `fm-fresh-start.sh`      | Decide when a second mate gets an automatic fresh start (idle, context, nightly) and run it through the persist-gated restart, or suggest one to the primary |
...
| `fm-lifecycle-lib.sh`    | Best-effort writer of the append-only `fm-lifecycle.v1` lifecycle event feed and its status transcription |
| `fm-lifecycle.sh`        | Backfill live tasks into the lifecycle event feed, or print its path |
