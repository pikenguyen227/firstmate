...
Before that return, teardown removes the home's own ignored files from the slot, which Treehouse would otherwise keep for the next lease; `bin/fm-teardown.sh` owns that removal and its unlanded-clone refusal.
