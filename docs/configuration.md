...
A home that used the earlier in-home pool at `state/treehouse-pool/` has its disposable slots removed at its next spawn, while a slot that is leased, in use, or holds unlanded work stays in place and is named on every spawn until the operator resolves it.
