...
Slots that secondmate clones created in the shared default pool before this rule are left untouched: a spawn handed one skips it, keeps holding it, and takes or creates a slot of its own clone; after three such skips it fails naming every rejected slot and its owning clone, which the operator removes deliberately (for example with `treehouse destroy`) once they hold no live work. [`docs/verification/secondmate-pool-per-home.md`](verification/secondmate-pool-per-home.md) owns live evidence against the real Treehouse.
