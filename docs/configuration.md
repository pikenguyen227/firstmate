...
- `context_percent` - the context trigger: percent of the model's context window in use, 10 to 99, default 70; `null` turns this trigger off. Unlike idle and nightly, it fires while the agent still holds in-flight work (live workers, open decisions, unread steering, undrained notifications), because the persist gate has the mate file that work before the relaunch; it still waits for the agent to be between turns, for a second mate to have no parent request still awaiting its reply, and for the cooldown.
...
Before that return, teardown removes the home's own ignored files from the slot, which Treehouse would otherwise keep for the next lease; `bin/fm-teardown.sh` owns that removal and its unlanded-clone refusal, and [`docs/verification/secondmate-slot-reuse.md`](verification/secondmate-slot-reuse.md) owns live evidence against the real Treehouse.
A secondmate home's workers get worktrees of that home's own project clones from a Treehouse pool of its own at `${XDG_STATE_HOME:-~/.local/state}/firstmate/treehouse-pools/<id>-<hash>`, outside every firstmate home, while the primary keeps Treehouse's configured default pool; `fm_treehouse_home_pool_root` in [`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh) owns the rule, and a spawn that reaches another clone's worktree stops with that clone named.
A pool outside the home keeps every worker's directory ancestry clear of the home's `CLAUDE.md` and supervisor `AGENTS.md`, which Claude would otherwise offer as an external import that a worker must decline; a spawn whose pool root or entered worktree sits under a firstmate home stops naming that home.
A home that used the earlier in-home pool at `state/treehouse-pool/` has its disposable slots removed at its next spawn, while a slot claimed by a task the home still records, leased, in use, or holding unlanded work stays in place and is named, with why, on every spawn until the operator resolves it; relaunching a task recorded in such a slot is refused, naming the home, so the operator lands or saves its work and respawns it.
Retiring a secondmate cleans both of its pools the same way and refuses, naming each slot, while any slot still holds work.
Slots that secondmate clones created in the shared default pool before this rule are left untouched: a spawn handed one skips it, keeps holding it, and takes or creates a slot of its own clone; when the slot handed out after three such retries is foreign too, it fails naming every rejected slot and its owning clone, which the operator removes deliberately (for example with `treehouse destroy`) once they hold no live work. [`docs/verification/secondmate-pool-per-home.md`](verification/secondmate-pool-per-home.md) owns live evidence against the real Treehouse.
...
When on and at least one rule exists, the tool sends the project name and only the brief's top-level `# Task` section (its `## Captain's intent` and `## Firstmate spec`) as state, never the scaffold boilerplate around it, and asks one Choice question whose options are every rule's `when` plus the fixed neutral option for no matching rule; the model never sees quota, catalogs, `why`, `use`, or approvals.
Job instructions may go to typesafe.ai for every project, including company projects, only when they carry no sensitive information, so a conservative pattern screen checks that exact text first for likely secrets, connection strings, private or link-local addresses, internal-looking hostnames, and any host outside a fixed list of well-known public domains, including a scheme-less `host:port` or `user@host` and any IPv4 literal outside loopback; a version or dist-tag after `@` (such as `@v4`, `@latest`, or `@18.2.0`) is not read as a host.
A screen match, a brief with no non-empty Task section, or a missing screen tool sends nothing and returns `escalate` with a `task text not sent:` reason that names only the kind of match, never the matched text; the screen prefers false positives, which cost only the ordinary intake.
...
The result is one of `clear` (a `profile:` line ready for `fm-spawn.sh`), `ambiguous` (confidence below the floor), `escalate` (an approval-gated rule, unverifiable rule floor, nothing rankable, a genuine tie, or task text that was not sent), or `error` (API, network, malformed response metadata, rendering, or quota-axi failure), and every one of them exits 0.
