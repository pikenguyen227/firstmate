...
. "/Users/pike/.no-mistakes/worktrees/81f2a0639ef5/01M395BBC9YW25EHG9YDYSPTNN/tests/lib.sh"
...
closed_gh() {  # gh reports the PR CLOSED (never merged)
  local c=$1 head=$2
...
SH
  cat > "$c/fakebin/gh" <<SH
#!/usr/bin/env bash
case "\${1:-} \${2:-}" in "pr view") case " \$* " in
  *"state,headRefOid,url"*) printf '%s\t%s\t%s\n' CLOSED '$head' https://github.com/example/repo/pull/7; exit 0;;
  *"headRefOid"*) printf '%s\n' '$head'; exit 0;; esac;; esac
echo "error: not found" >&2; exit 1
SH
  chmod +x "$c/fakebin/gh-axi" "$c/fakebin/gh"
}
show() { echo "--- $1: teardown rc=$2; persisted task.torn_down record:"; jq -c 'select(.type=="task.torn_down") | .data' "$3/data/lifecycle/events.v1.jsonl"; }

# A: pushed ship, PR closed without merging (gh says CLOSED)
c=$(make_case manual-closed); write_meta "$c" no-mistakes ship
wt_commit_file "$c" feature.txt hi "closed work"; git -C "$c/wt" push -q origin fm/task-x1; git -C "$c/project" fetch -q origin
append_pr_meta_for_current_head "$c"; closed_gh "$c" "$(git -C "$c/wt" rev-parse HEAD)"
run_teardown "$c" >"$c/out" 2>&1; show "A closed-unmerged PR (gh CLOSED)" $? "$c"

# B: forced teardown even though a merge marker exists for the exact PR -> must stay unknown
c=$(make_case manual-forced); write_meta "$c" no-mistakes ship
wt_commit_file "$c" feature.txt hi "work"; append_pr_meta_for_current_head "$c"
bash -c '. "$1/bin/fm-pr-lib.sh"; fm_pr_poll_merge_mark_notified "$2" task-x1 github github.com example/repo 7' _ "$ROOT" "$c/state"
run_teardown "$c" --force >"$c/out" 2>&1; show "B forced teardown with marker" $? "$c"

# C: no pr= in meta, merged PR discovered by branch -> merged
c=$(make_case manual-discovered); write_meta "$c" no-mistakes ship
wt_commit_file "$c" feature.txt hi "add"; add_gh_pr_merged_for_head "$c" "$(git -C "$c/wt" rev-parse HEAD)"
run_teardown "$c" >"$c/out" 2>&1; show "C merged PR discovered by branch (no pr= recorded)" $? "$c"

# D: ship with no PR at all, pushed branch -> unknown, no pr
c=$(make_case manual-nopr); write_meta "$c" no-mistakes ship
wt_commit_file "$c" feature.txt hi "x"; git -C "$c/wt" push -q origin fm/task-x1; git -C "$c/project" fetch -q origin
run_teardown "$c" >"$c/out" 2>&1; show "D pushed ship, no PR" $? "$c"
