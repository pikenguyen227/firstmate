...
Seeding places a leased home, or an explicit home it clones because the path did not exist, at the primary's current default-branch commit, the same target the pre-launch sync below follows, so the first launch never meets a divergence; if it cannot, the seed fails and rolls back, while an explicitly given home that already existed is never moved and a mismatch is only warned ([`bin/fm-home-seed.sh`](../../../bin/fm-home-seed.sh)).
...
Automatic fresh starts of an idle, context-full, or nightly mate go through that same persist gate; `bin/fm-fresh-start.sh` owns when, and [`docs/configuration.md`](../../../docs/configuration.md#automatic-fresh-starts-configfresh-startjson) owns the switch.
...
A non-forced retirement of a leased home refuses, naming the clone and leaving the home intact, while any project clone in it holds uncommitted, stashed, or unpushed work, and any process still running inside the home is reaped before its files are removed.
